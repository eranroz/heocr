...
def findArea(regions):
    ...
    [hist, bins] = np.histogram(areas,
                                bins=range(int(round(min(areas), -1)), int(round(max(areas), -1)) + length_of_bin,
                                           length_of_bin))
    index = np.argmax(hist)
    height = bins[index + 1]
    return height
...
def getPictureInTheimage(regions, image):
    ...
    for region in regions:
        ...
        cureent_he = max_pic_r - min_pic_r
        current_width = max_pic_c - min_pic_c
        if (region.area < area_threshold * 1.5 or height_threshold * 1.5 > cureent_he) or current_width > (
                    image_width / 2) * .8:
            continue
        regions_after_filters.append(region)
    return regions_after_filters
...
def inRegion(letter, region):
    ...
    buffer_val = 100
    if (
                                minc_letter + buffer_val >= minc and maxc_letter - buffer_val <= maxc and minr_letter + buffer_val >= minr and maxr_letter - buffer_val <= maxr ):
        return True
    return False
...
def OCRWith_SVM(letters):
    ...
    y_pred = svm_predict(data)
    return y_pred


def imageSegmentation(image_name):
    """order all the letters

    :param image_name:
    :return:
    """
    # Read image
    img_data = imread(image_name)
